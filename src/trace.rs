...
pub enum TraceDirection { Transmit, Receive }
...
{
    fn trace(&mut self, event: TraceEvent<'_>) { self(event); }
}
...
impl<D, S> Tracer<D, S> {
    pub const fn new(inner: D, sink: S) -> Self { Self { inner, sink } }
    pub fn inner(&self) -> &D { &self.inner }
    pub fn inner_mut(&mut self) -> &mut D { &mut self.inner }
    pub fn sink(&self) -> &S { &self.sink }
    pub fn sink_mut(&mut self) -> &mut S { &mut self.sink }
    pub fn into_parts(self) -> (D, S) { (self.inner, self.sink) }
...
impl<D: GnetFrameDevice, S: TraceSink> GnetFrameDevice for Tracer<D, S> {
    fn transmit_frame(&mut self, frame: GnetFrame) -> Result<()> {
        self.sink.trace(Self::event(TraceDirection::Transmit, &frame));
    ...
    fn receive_frame(&mut self) -> Result<Option<GnetFrame>> {
        ...
        if let Some(ref frame) = frame {
            self.sink.trace(Self::event(TraceDirection::Receive, frame));
        }
        Ok(frame)
    }
}
