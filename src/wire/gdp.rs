...
impl GdpType {
    pub const fn to_wire(self) -> u8 {
        match self { Self::Gctl => 0x1, Self::Gts => 0x2, Self::Reserved(v) => v & 0x0f }
    }
    pub const fn from_wire(v: u8) -> Self {
        match v & 0x0f { 1 => Self::Gctl, 2 => Self::Gts, x => Self::Reserved(x) }
    }
}
...
impl GdpHeader {
    pub fn global(packet_type:GdpType,size_class:SizeClass,hop_limit:u8,source:GdpAddress,destination:GdpAddress)->Self {
        Self{version:0,packet_type,size_class,hop_limit,addresses:GdpAddresses::Global{destination,source}}
    }
    pub fn local(packet_type:GdpType,size_class:SizeClass,hop_limit:u8,prefix:u64,source:u16,destination:u16)->Result<Self>{
        if hop_limit>15 { return Err(Error::InvalidField); }
        Ok(Self{version:0,packet_type,size_class,hop_limit,addresses:GdpAddresses::Local{destination,source,prefix:prefix & !0xffff}})
    }
    ...
    pub fn crc8(&self, cfg:GdpWireConfig)->u8 {
        let mut c=Crc8::new();
        c.update_bits((self.version&0x03) as u64,2);
        c.update_bits(self.packet_type.to_wire() as u64,4);
        c.update_bits(self.size_class as u8 as u64,4);
        c.update_bit(cfg.form_bit(self.addresses.form()));
        match self.addresses {
            GdpAddresses::Global{destination,source}=>{ c.update_bits(destination.0,64); c.update_bits(source.0,64); }
            GdpAddresses::Local{destination,source,..}=>{ c.update_bits(destination as u64,16); c.update_bits(source as u64,16); }
        }
        c.finalize()
    }
    pub fn encode(&self,cfg:GdpWireConfig)->Result<Vec<u8>> {
        ...
        let form_bit = cfg.form_bit(self.addresses.form()) as u32;
        let crc=self.crc8(cfg) as u32;
        let hop = match self.addresses { GdpAddresses::Global{..}=>self.hop_limit as u32, GdpAddresses::Local{..}=>(self.hop_limit & 0xf) as u32 };
        ...
        let mut out=Vec::with_capacity(self.header_len()); out.extend_from_slice(&word1.to_be_bytes());
        match self.addresses {
            GdpAddresses::Global{destination,source}=>{ out.extend_from_slice(&destination.0.to_be_bytes()); out.extend_from_slice(&source.0.to_be_bytes()); }
            GdpAddresses::Local{destination,source,..}=>{ let w=((destination as u32)<<16)|source as u32; out.extend_from_slice(&w.to_be_bytes()); }
        }
        Ok(out)
    }
    pub fn decode(buf:&[u8],cfg:GdpWireConfig,local_prefix:u64)->Result<Self>{
        if buf.len()<4 {return Err(Error::InvalidLength)}
        let w=u32::from_be_bytes(buf[0..4].try_into().unwrap());
        let version=((w>>30)&3) as u8; let packet_type=GdpType::from_wire(((w>>26)&0xf) as u8); let size_class=SizeClass::from_wire(((w>>22)&0xf) as u8)?;
        let form=cfg.form_from_bit(((w>>21)&1)!=0); let received_crc=((w>>8)&0xff) as u8;
        let header=match form {
            AddressForm::Global=>{ if buf.len()<20{return Err(Error::InvalidLength)}; let hop=(w&0xff) as u8; let destination=GdpAddress(u64::from_be_bytes(buf[4..12].try_into().unwrap())); let source=GdpAddress(u64::from_be_bytes(buf[12..20].try_into().unwrap())); Self{version,packet_type,size_class,hop_limit:hop,addresses:GdpAddresses::Global{destination,source}} }
            AddressForm::Local=>{ if buf.len()<8{return Err(Error::InvalidLength)}; let hop=(w&0xf) as u8; let ids=u32::from_be_bytes(buf[4..8].try_into().unwrap()); Self{version,packet_type,size_class,hop_limit:hop,addresses:GdpAddresses::Local{destination:(ids>>16) as u16,source:ids as u16,prefix:local_prefix & !0xffff}} }
        };
        if header.crc8(cfg)!=received_crc { return Err(Error::InvalidCrc); }
        Ok(header)
    }
}
...
fn compare_with_p4(buf: &[u8], cfg: GdpWireConfig, rust: &GdpPacket) -> Result<()> {
    ...
    use crate::p4_gdp_compare::{parse_and_validate, ParsedAddresses};

    let p4 = parse_and_validate(buf).map_err(|_| Error::InvalidField)?;
