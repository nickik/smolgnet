...
fn assert_payload_crc_contract(profile: StreamProfile) {
    ...
    if profile.unreliable && profile.unchecked_payload {
        assert!(decoded.is_ok(), "unchecked datagram payload must not be covered by CRC: {profile:?}");
    } else {
        assert_eq!(
            decoded.unwrap_err(),
            Error::InvalidCrc,
...
fn one_tunnel_exercises_every_legal_stream_profile_and_rejects_every_illegal_one() {
    ...
    assert!(legal.iter().filter(|p| !p.unreliable).all(|p| !p.sequenced && !p.unchecked_payload));
    assert!(illegal.iter().all(|p| !p.unreliable && (p.sequenced || p.unchecked_payload)));
    ...
    for profile in legal.iter().copied() {
        assert_eq!(StreamProfile::from_wire(profile.to_wire().unwrap()).unwrap(), profile);
    ...
    for profile in illegal.iter().copied() {
        assert_eq!(a.open_stream(ah, profile).unwrap_err(), Error::ProfileViolation);
        assert_eq!(b.open_stream(bh, profile).unwrap_err(), Error::ProfileViolation);
    }
    ...
    for index in schedule {
        ...
        let payload = rng.fill(payload_len_for(stream.profile, &mut rng));
