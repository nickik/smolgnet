...
use smolgnet::p4_gdp_compare::parse_and_validate;
...
fn compare_handwritten_and_x4c_gdp_decode() {
    ...
    for case in packet_cases() {
        let handwritten_header = bench(iterations, || {
            let h = GdpHeader::decode(black_box(case.bytes.as_slice()), cfg, case.local_prefix).unwrap();
            black_box(h);
        });
        ...
        let handwritten_packet = bench(iterations, || {
            let p = GdpPacket::decode(black_box(case.bytes.as_slice()), cfg, case.local_prefix).unwrap();
            black_box(p);
        });
        let p4_packet = bench(iterations, || {
            let parsed = parse_and_validate(black_box(case.bytes.as_slice())).unwrap();
            let header_len = if parsed.local_form { 8 } else { 20 };
            let payload = black_box(case.bytes[header_len..].to_vec());
            black_box((parsed, payload));
        });

        let hw_ns = ns_per_packet(handwritten_packet, iterations);
        let p4_ns = ns_per_packet(p4_packet, iterations);
        println!(
            "packet,{},{:.2},{:.2},{:.3},{:.3},{:.3}",
            case.name,
            hw_ns,
            p4_ns,
            p4_ns / hw_ns,
            mpps(handwritten_packet, iterations),
            mpps(p4_packet, iterations)
        );
    }
}
