...
fn drain_round(tx: &mut DlpLink, rx: &mut DlpLink, count: usize) -> Vec<(u8, u8, Vcid)> {
    let mut out = Vec::new();
    for _ in 0..count {
        let frame = tx.poll_tx_frame().unwrap().unwrap();
        let vcid = frame.vcid;
        let packet = rx.receive_frame(frame).unwrap();
...
fn vc4_sustained_busy_flows_rotate_one_packet_quantum() {
    ...
    while scheduler.queued_packets() != 0 {
        ...
        history.extend(drain_round(&mut tx, &mut rx, admitted));
    }
...
fn vc2_admits_exactly_one_flow_per_round() {
    ...
    for id in 0..4u8 {
        ...
        assert_eq!(drain_round(&mut tx, &mut rx, 1), vec![(id, 0, Vcid::VC1)]);
    }
}
